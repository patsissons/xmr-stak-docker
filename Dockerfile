...
RUN set -x \
  ...
  && apt-get update \
  && apt-get install -qq --no-install-recommends -y ca-certificates libhwloc5 libmicrohttpd12 libssl1.0.0 libuv1 wget xz-utils \
  ...
  && ./${AMDGPU_DRIVER_NAME}/amdgpu-pro-install -y --headless --opencl=pal,legacy \
